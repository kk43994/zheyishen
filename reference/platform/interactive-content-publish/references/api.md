...
| `app_id` | 更新时是 | 更新时必填 |
| `desc` | 否 | 互动空间描述 |
| `ai_enabled` | 否 | 是否开启互动空间 AI 调用能力；开启后后端会校验当前账号已配置火山方舟 API Key 或平台服务账号 |
...
- `ai_enabled` 使用发布流程中用户确认的 AI 能力开关
- zip 包内命中 `tt.callAIChatCompletion`、`tt.callAIGenerateImage`、`tt.callAIGenerateVideo` 任意一个时建议传 `true`
- 三个 API 都未命中时建议传 `false`
- 不要把 `tt.chooseImage`、`tt.chooseVideo`、`tt.getRecorderManager` 或 `/mgplatform/api/ark/*` 作为 AI 调用特征
- 当 `ai_enabled=true` 时，后端会校验当前账号是否已完成 AI 调用配置。如果返回“请先配置 AI 调用 API Key”、错误码 `20107`，或错误信息包含 AI/API Key/服务账号/实名/权限相关内容，需要提示用户先配置火山方舟 API Key 或按平台指引完成服务账号配置，并停止后续 `submit_audit_game_app`
