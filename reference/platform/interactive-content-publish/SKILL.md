...
5. **判断 AI 能力建议**：扫描 zip 包内文本文件，只把以下三个前端 API 视为 AI 调用特征：
   - `tt.callAIChatCompletion`
   - `tt.callAIGenerateImage`
   - `tt.callAIGenerateVideo`

   命中任意一个则建议开启 AI 能力；三个都未命中则建议关闭 AI 能力。不要把 `tt.chooseImage`、`tt.chooseVideo`、`tt.getRecorderManager` 或 `/mgplatform/api/ark/*` 等辅助能力/服务端路径算作 AI 调用特征。
...
> - 🖼️ 图标：`<用户提供的图标文件名>`
> - 📱 体验方式：`<竖屏/横屏>`
> - 🤖 AI 能力：`<建议开启/建议关闭>`（依据：`<命中的 tt.callAI* API；未命中则写未发现三类 AI API>`）
> - 🔧 产物来源：`<当前AI工具名称>`（如能识别当前工具名称则自动填入，如 “Trae Solo”、“Cursor” 等；无法确定则留空）
>
> 是否需要修改？
>
> - 确认无误 → 回复"确认"
> - 需要修改 → 直接输入修改内容，如"名称改成 xxx"、"描述改成 xxx"、"AI能力改成开启/关闭"或"改成横屏"
...
- `ai_enabled` 使用 Step 3 中用户确认的 AI 能力开关；确认开启传 `true`，确认关闭传 `false`，不要省略该字段，避免更新时沿用旧开关
...
如果 `modify_game_app` 返回错误，且错误信息包含 `AI`、`API Key`、`服务账号`、`实名`、`权限`、`ErrAIAPIKeyNotConfigured`，或错误码为后端 AI API Key 未配置错误（如 `20107`），必须停止后续流程，不要继续调用 `submit_audit_game_app`。

向用户给出友好提示：

> 当前账号还没有完成 AI 调用配置，已停止提审。请先在互动空间平台的 AI 服务配置中配置火山方舟 API Key，或按页面指引完成平台服务账号/手机号验证/火山实名；完成后再重新执行发布。若本作品不需要 AI 能力，也可以回复"AI能力改成关闭"后重新发布。
