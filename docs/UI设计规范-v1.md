...
| 当铺 | `shop` / `renderShop()` | 三件、价格、逐项试穿、共用货架、纸质价签、零钱落盒、撕签、失败空响与重整落页 | 已完成；后续只按实机校准动效节拍 |
| 留灯间/里屋 | `specialRoom` / `renderSpecialRoom()` | 两类实体房间、托盘/挂钩陈列、共享身体预览、按住交换、不同代价；内部倾向已隐藏 | 补齐更细的年龄提前与身份类代价预演 |
