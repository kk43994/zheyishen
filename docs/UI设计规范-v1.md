...
《这一身》的 UI 应该像一份在夜里被不断填写、折叠、盖章、夹入新物证的人生档案，而不是一套奇幻卡牌框、霓虹 HUD 或复古报纸皮肤。
...
**画面：** 深墨夜色里放一张未展开的档案封皮。主角不是站在血月海报前，而是以 40×56 的初始空身像素剪影站在封皮外，身后只有月白呼吸留下的一小点。标题像被铅字压进封皮，旧红只用于“身”字的一笔或封卷章。
...
1. 独立像素物件和名称。
2. 穿到主角身上会改变什么；用一张共享主角剪影做悬停/点按预览。
3. `得到` 与 `留下` 两条机械信息；不用正负道德色。
4. 品级与一句物件原话。

**桌面：** 三座并列物证台，之间只用竖向档案分隔线。共享主角预览站在三座台后方，指向某件时才叠加外貌变化。

**手机：** 三件物证纵向排列成三条完整记录，每条约 150–168 像素高，三件必须同屏可比较。点击某条只展开它的外貌预览，不把其他两条推走。
...
**布局：** 三件物品直接摆在同一货架或柜台上，每件只有一张挂签；价签是第一层信息，效果是第二层，讽刺文案是第三层。零钱固定在柜台账本右上角。

**动作：**

- 购买：拿起物件，零钱落入铁盒，价签被撕下留下两个针孔。
- 重整：使用熟悉的循环箭头图标，旁边显示成本 `2`，桌面悬停显示“换一批”。
...
| 手机竖屏 | 360×640 | 按 DPR 整数倍后适配 CSS | 单手移动、三项纵向比较 |
...
移动端正文不得低于 12 CSS 像素的视觉尺寸；长词必须换行，不挤压图标或相邻信息。
...
缺失独立暂停状态；阶段转场依赖 `battle + transitionTimer`；出生、命运和结果各自在单一状态中用多个布尔量表达子阶段。
...
| 首件/普通奖励 | `itemReward` / `renderItemReward()`、`drawItemChoice()` | 三选一、不可跳过、正负效果 | 去掉三张厚卡；增加共享身体预览；手机改三条纵向记录 |
| 战斗 | `battle` / `renderBattle()` | 世界优先、自动攻击、外号字章 | 重排贴边 HUD，清空主角周围文字区，移除全宽底栏 |
| 一口气 HUD | `renderBattleOverlay()` | 相对基础值的构筑表达 | 收敛为劲/速/程；详细公式放展开档案 |
...
| 阶段转场 | `beginStageTransition()`、`transitionTimer`、`renderWorld()` | 世界连续渐变、仍可移动 | 3.4 秒扩为三段视觉协议，最终匹配 15–20 秒正典 |
...
| 命运结果 | `renderFateResultCard()` | 结果、玩家原话、属性变化 | 改为窄回执；删除五毒增量显示 |
| 当铺 | `shop` / `renderShop()` | 三件、价格、重整、离开 | 卡片改共用货架；刷新/离开改图标命令；购买用价签反馈 |
| 留灯间/里屋 | `specialRoom` / `renderSpecialRoom()` | 两类房、光色、三件、不同代价 | 两者做成真实空间；移除物品卡与“空肺/窗灯”调试数值 |
| 暂停 | 无 | 无 | 新增正交暂停覆盖层、设置、构筑与退出保护 |
| 结算 | `result` / `renderResult()` | 最终人偶、构筑、统计、再来一次 | 首屏叙事优先；增加五毒揭示与命运时间线；实现“身→生” |

### 7.3 现状与正典冲突

1. `renderTitle()` 当前是营销首页，而非游戏封面。
2. `renderOriginLoading()`、`renderOrigin()` 与 `renderFateEvent()` 直接显示 AI/GPT/本地保底，破坏无感生成和世界观。
3. `renderBattleOverlay()` 与 `renderPoisonStrip()` 公开五毒数值，违反“五毒先身体化、结算才揭示”。
4. `renderFateResultCard()` 在选择后直接显示毒性增量，容易把回应变成道德考试。
5. `transitionTimer = 3.4` 与百科的 15–20 秒过渡带不一致。
6. `renderItemReward()`、`renderShop()`、`renderSpecialRoom()` 复用同一组三卡面板，使道具、当铺和两扇门失去各自的叙事空间。
7. `src/style.css` 与 `renderBackground()` 大量使用径向/线性渐变，与正典“有限调色板、无渐变”冲突。
8. 当前固定 360×640 画布，没有 16:9 桌面布局分支。
9. 7–11 像素 Canvas 文本在 1× CSS 尺寸下过小；手机正文需要提升到 12 像素视觉尺寸。
10. 没有暂停、设置、声音分层、减少动态和完整档案查询。
...
- 三选一使用共享身体预览，替换复用的大卡面板。
- 当铺、留灯间、里屋分别实现货架、暖屋、冷屋三种空间结构。
...
- 结算增加五毒首次揭示、命运回执、道具时间线与“身→生”。
- 接入分层音量、振动、减少动态、文字大小、高对比和左手摇杆。
