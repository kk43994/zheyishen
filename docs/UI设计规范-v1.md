...
《这一身》的 UI 应该像一条正在被走完的人生路径：生活空间连续变化，普通物件逐段接棒，穿在身上的东西成为身体证据。它不是血月海报、档案皮肤、奇幻卡牌框、霓虹 HUD 或复古报纸。

月白呼吸是贯穿全局的动作线索；冷病房蓝、雨衣黄和不同年龄的生活材质负责区分状态。“档案”只在事实需要被确认时局部出现：出生是登记表，命运是无法撤回的事实页，结算是封卷。战斗、奖励、当铺和房间不套纸张皮肤，只留下安静、准确、能一眼读懂的身体与空间信息。
...
材质随人生渐变，但核心布局位置不漂移。玩家应感到生活空间在连续变旧、变冷、变空，而不是每章换了一套 UI 主题包。
...
2. 一句主题句：“这一身，最后都穿成了这一生。”
3. 唯一主动作：“开始呼吸”。
4. 右上角只放声音、辅助设置两个图标；桌面悬停、移动端长按显示名称。

**画面：** 开屏是一条从童年、学校、工厂、工位、家庭走向病房的连续人生路径。六段生活空间以地面、灯光和普通物件逐段接棒，初始空身像素人偶站在路上，身边只有一小口月白呼吸。标题保持素白，不使用血月、红字残影或档案封皮；冷病房蓝、雨衣黄和月白构成主要识别色。标题页可以暗，但必须让人生阶段和行进方向清楚可读。

**交互与动效：** 路上的阶段标记缓慢前行，月白呼吸在主角身边明灭。点击后画面从呼吸所在的中轴向两侧揭开，进入第一次出生登记；按钮不弹跳，只在按下时下沉 1 像素并点亮两侧呼吸导轨。首次交互同时发起出生生成请求。
...
**外号生成合同：** 外号为 2–7 个汉字，必须是一听就在形容或指认某个人的称谓。把它填进“喂，___，过来”时应当像在叫人；裸的动作、家务、任务、物件清单和事件标题均不合格，例如“数凉席”“捡煤核”“背课文”。动作或物件只能作为来由，最终必须转写成人称化、特征化的外号。运行时必须二次校验；不合格输出视同本次出生未落档，继续重写，不能把问题外号带入字章。
...
**桌面端实现约束：** 保持同一套 9:16 逻辑画布，不把战场横向拉伸；在 `700px` 以上且高度充足时按可用高度放大，宽度上限为 `456px`。桌面不能退回 `360px` 原始逻辑宽度，否则正文与 HUD 会显得过小。两侧空间只延续当前阶段的低对比环境与暗角，不新增脱离主流程的面板，也不固定成档案桌背景。
...
**设置至少包含：** 总音量、环境声、人声、物件/战斗音效、振动、减少动态与高对比。当前没有独立音乐轨，不显示无效的音乐滑杆；动态摇杆随落指位置生成，左右手都能直接使用，不显示虚假的左右手切换。文字大小需要整套布局变体后再开放，不能只把字体放大造成溢出。二元设置使用开关，音量使用滑杆，页签使用图标加短标签。
...
| --- | --- | --- | --- |
| 封面 | `title` / `renderTitle()` | 标题、初始人偶、开始动作 | 已完成：连续人生路径、月白呼吸和“开始呼吸”；无血月与全局档案封皮 |
...
| 出生字章 | `renderOriginBadge()` | 完整外号常驻可读、出生缘由可展开 | 已完成：紧凑态为透明红章身份牌，扩展态为窄幅复写纸档案页并避开战场中心 |
...
| 暂停 | `paused` / `renderPauseOverlay()` | 原位冻结与恢复、构筑/出生/命运页签、总声/环境/人声/音效四路滑杆、振动、减少动态、高对比、按住退出 | 已完成真实音频管线分层；文字大小待完整布局变体，不提供无效音乐或左右手开关 |
...
7. 战场不加载循环地砖，也不生成随镜头移动的规则刻痕或重复摆件。青年关禁止用站台横线、枕木短线或扫描竖线暗示车站；战斗区四周禁止叠加多重内缩暗边。章节底色与非交互气氛稳定在屏幕层，只有敌人、掉落物和可交互实体使用世界坐标。
8. 弹体 UI 与渲染遵守“一颗弹一个 form、一种 tail、多组 mechanics flags”；不得用多张完整弹体贴图表现道具叠加。视觉代次 `generation` 不得被用来清空机制，只有 `splitDepth` 可以阻止递归分裂。
...
- 程序化关键音效、总声/环境/人声/音效分层、振动、减少动态和高对比已接入；文字大小需与完整布局变体一起实现。动态摇杆随落指位置生成，天然支持左右手。

**验收：** 暂停可从战斗和命运页进入并原位恢复；通关与死亡首屏叙事不同；关闭颜色后仍能辨认所有成本和选择。

## 9. 最终 UI 验收句

玩家截掉所有文字后，仍能从“道路向前、空间变老、事实落下、物件被穿上、房间被进入、最后一口气熄灭”看懂这一局的一生；登记页与结算可以保留档案动作，但玩家在其余页面不会觉得自己始终困在一份表格里。
