...
- **生成式命运事件连成连续剧**：上一次的选择成为这一次事件的起因，玩家亲口说过的话会被后续事件承认并延续；回应改变五毒与后续构筑
- **当铺 + 随机刷新的留灯间/里屋实体门**（限时存在，走进去触发）：里屋卖代价，留灯间存放被爱过的证据
- **美术管线**：敌怪（19 种，五行动作图集）、场景摆设（六章 24 件）、道具图标走「生图基底 + 程序规整」混合管线；主角人偶纯程序化（形变/穿戴依赖代码控帧）；AI 只能选择白名单模板，不能自由生成图像或数值
