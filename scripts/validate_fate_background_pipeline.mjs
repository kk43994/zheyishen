...
assert(/attempt <= 2/.test(generateAIFate), '命运牌后台生成最多允许两轮定向尝试');
...
assert(/buildFreeFateMechanicBudget/.test(generateAIFreeFate), '亲口回应必须使用带载体约束和去重的白名单机械预算');
assert(/validateFreeFateResponse/.test(generateAIFreeFate), '亲口回应必须经过剧情证据编译器');
