...
# 平台包硬上限 20 MiB：public 里保留声音母版，只压缩 dist 的旁白副本。
# 对白 24 kbps 单声道（22.05 kHz）——用户两次裁决「不需要那么清楚的语音、可以再差一点」，
# 换更短的首次加载与更小的解码开销。配乐与环境床禁止在这里二次有损转码：它们已经
# 从 WAV 母版一次直编（配乐 64 kbps / 环境床 96 kbps）；再转 24k 的全曲 SI-SDR 实测只剩 21.05/15.42 dB，
# 会把砂化和瞬态毛刺听成循环撕裂。当前包体余量足够保留一次编码成品。
...
npm run validate:release-audio
