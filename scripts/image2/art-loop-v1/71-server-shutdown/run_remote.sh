...
job_dir="$pipeline_dir/art-loop-v1/71-server-shutdown"
...
image_args=(
  ...
  --image "$job_dir/input/02-approved-hero-ground-anchors.png"
  --image "$job_dir/input/03-current-failed-permanent-devices.png"
  --image "$job_dir/input/04-current-failed-fullscreen-shutdown.png"
  --image "$job_dir/input/05-rejected-paper-source.png"
)

if [[ "$version" != "v1" ]]; then
  previous_version="v$((10#${version#v} - 1))"
  previous_output="$job_dir/output/$previous_version/71-server-shutdown-$previous_version.png"
...
"$pipeline_dir/venv/bin/python" "$pipeline_dir/image_gen.py" edit \
  ...
  --no-augment \
  "${image_args[@]}" \
  --prompt-file "$job_dir/prompt-trigger-$version-draft.txt" \
  --out "$output_dir/71-server-shutdown-$version.png" \
  --force
