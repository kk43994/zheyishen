...
job_dir="$pipeline_dir/art-loop-v1/12-first-salary"
...
"$pipeline_dir/venv/bin/python" "$pipeline_dir/image_gen.py" edit \
  ...
  --output-format png \
  --no-augment \
  --image "$job_dir/input/canonical-style-board.png" \
  --image "$job_dir/input/12-first-salary-current.png" \
  --prompt-file "$job_dir/prompt-$version.txt" \
  --out "$output_dir/12-first-salary-$version.png" \
  --force
