...
output_dir="$job_dir/output/v2.1"
...
"$pipeline_dir/venv/bin/python" "$pipeline_dir/image_gen.py" edit \
  ...
  --image "$job_dir/input/05-rejected-v2-ambiguous-species.png" \
  --prompt-file "$job_dir/prompt-v2.1-draft.txt" \
  --out "$output_dir/60-momo-avatar-v2.1.png" \
  --force
