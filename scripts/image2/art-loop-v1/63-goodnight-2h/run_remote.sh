...
job_dir="$pipeline_dir/art-loop-v1/63-goodnight-2h"
...
image_args=(
  ...
  --image "$job_dir/input/03-current-failed-blue-portals.png"
  --image "$job_dir/input/04-target-phone-shadow-geometry.png"
)

if [[ "$version" != "v1" ]]; then
  previous_version="v$((10#${version#v} - 1))"
  previous_output="$job_dir/output/$previous_version/63-goodnight-2h-$previous_version.png"
...
"$pipeline_dir/venv/bin/python" "$pipeline_dir/image_gen.py" edit \
  ...
  --out "$output_dir/63-goodnight-2h-$version.png" \
  --force
