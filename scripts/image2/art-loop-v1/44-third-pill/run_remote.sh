...
job_dir="$pipeline_dir/art-loop-v1/44-third-pill"
...
"$pipeline_dir/venv/bin/python" "$pipeline_dir/image_gen.py" edit \
  ...
  --image "$job_dir/input/02-current-failed-44-third-pill.png" \
  --image "$job_dir/input/03-approved-hero-4dir.png" \
  --image "$job_dir/input/04-approved-head-torso-parts.png" \
  --prompt-file "$job_dir/prompt-$version.txt" \
  --out "$output_dir/44-third-pill-$version.png" \
  --force
