...
requireToken(gallery, '15 个阶段形态 · 41 条独立动作 · 164 帧', '百科 Boss 动画统计与运行时图集不一致');
requireToken(wiki, '六章大小 Boss 当前共 15 个阶段形态、41 条专属动作、164 帧', '百科世界观仍在使用旧 Boss 动画总数');
requireToken(wiki, '仅统计六只章节大 Boss：9 个阶段形态 · 31 条独立动作 · 每条 4 帧', '百科没有明确区分章节大 Boss 子集与大小 Boss 总表');
...
checks += 1;
if (actionCaptions.length !== 41) errors.push(`百科应有 41 条 Boss 动作条，实际 ${actionCaptions.length}`);

// 追加技能图（*-extra-skills）挂在既有形态名下，不计入阶段形态数。
const formCount = Object.keys(manifest.assets).filter((a) => !a.endsWith('-extra-skills')).length;
const skillCount = Object.keys(manifest.skills).length;
const frameCount = skillCount * 4;
checks += 3;
if (formCount !== 15) errors.push(`Boss 技能清单阶段形态应为 15，实际 ${formCount}`);
if (skillCount !== 41) errors.push(`Boss 技能清单动作应为 41，实际 ${skillCount}`);
if (frameCount !== 164) errors.push(`Boss 技能清单帧数应为 164，实际 ${frameCount}`);
